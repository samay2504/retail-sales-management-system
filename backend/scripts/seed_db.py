...
REGIONS = ["North", "South", "East", "West", "Central", "Northeast", "Southeast", "Northwest", "Southwest", "Midwest"]
GENDERS = ["Male", "Female", "Other", "Prefer not to say"]
CATEGORIES = [
    "Electronics", "Smartphones", "Laptops & Computers", "Smart Home Devices", "Audio Equipment",
    "Clothing", "Men's Fashion", "Women's Fashion", "Children's Clothing", "Accessories",
    "Food & Beverage", "Organic Foods", "Snacks & Confectionery", "Beverages", "Gourmet Foods",
    "Home & Garden", "Furniture", "Kitchen Appliances", "Gardening Tools", "Home Decor",
    "Sports & Outdoors", "Fitness Equipment", "Camping Gear", "Sports Apparel", "Bicycles",
    "Books & Media", "Fiction Books", "Non-Fiction Books", "Music & Movies", "Educational Materials",
    "Toys & Games", "Board Games", "Video Games", "Children's Toys", "Puzzles",
    "Health & Beauty", "Skincare", "Makeup", "Personal Care", "Supplements",
    "Automotive", "Car Parts", "Tools", "Car Accessories", "Maintenance Products",
    "Office Supplies", "Stationery", "Office Furniture", "Technology Accessories", "Business Equipment"
]

TAGS_POOL = [
    "bestseller", "clearance", "seasonal", "premium", "eco-friendly",
    "new-arrival", "limited-edition", "sale", "popular", "trending",
    "discounted", "hot-deal", "featured", "recommended", "exclusive",
    "bundle", "gift-idea", "holiday-special", "flash-sale", "pre-order",
    "handmade", "organic", "sustainable", "luxury", "budget-friendly",
    "top-rated", "customer-favorite", "award-winning", "imported", "local"
]

PAYMENT_METHODS = [
    "Credit Card", "Visa", "MasterCard", "American Express", 
    "Debit Card", "Cash", "Digital Wallet", "PayPal", "Apple Pay", 
    "Google Pay", "Bank Transfer", "Wire Transfer", "Cryptocurrency", 
    "Gift Card", "Store Credit", "Buy Now Pay Later"
]
...
EMPLOYEE_FIRST = [
    ...
    "Ethan", "Fiona", "George", "Hannah", "Isaac", "Julia", "Kevin", "Lucy", "Mason", "Nina"
]

EMPLOYEE_LAST = [
    "Anderson", "Baker", "Carter", "Davis", "Evans", "Foster", "Gray", "Hughes",
    "Jackson", "King", "Lee", "Martin", "Nelson", "Owen", "Parker", "Quinn",
    "Roberts", "Stone", "Taylor", "White", "Young", "Adams", "Brooks", "Clark"
]
...
def generate_transaction() -> dict:
    ...
    discount_percentage = random.choices(
        ...
        weights=[50, 20, 15, 8, 4, 2, 0.5, 0.3, 0.2]
    )[0]
    ...
    tags = ", ".join(random.sample(available_tags, min(num_tags, len(available_tags)))) if num_tags > 0 else ""
...
async def seed_database(num_records: int = 1000) -> None:
    ...
    print(f"📊 This will create a comprehensive dataset for testing edge cases.")
    ...
    async with AsyncSessionLocal() as session:
        ...
        for batch_num in range(total_batches):
            ...
            print(f"  📝 Progress: {progress}/{num_records} ({percentage:.1f}%) - Batch {batch_num + 1}/{total_batches} completed")
        ...
        # Count by region
        result = await session.execute(
            text("SELECT customer_region, COUNT(*) as count FROM transactions GROUP BY customer_region ORDER BY count DESC")
        )
        ...
        # Count by category
        result = await session.execute(
            text("SELECT product_category, COUNT(*) as count FROM transactions GROUP BY product_category ORDER BY count DESC LIMIT 5")
        )
        ...
        # Total revenue
        result = await session.execute(
            text("SELECT SUM(final_amount) FROM transactions")
        )
        ...
        # Average transaction
        result = await session.execute(
            text("SELECT AVG(final_amount) FROM transactions")
        )
