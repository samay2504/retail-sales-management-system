...
class QueryBuilder:
    ...
    def _apply_search(self, query: Select, search_term: str) -> Tuple[Select, bool]:
        ...
        if is_postgres:
            ...
            query = query.where(
                Transaction.id.in_(
                    select(text("id"))
                    .select_from(text(f"({fts_subquery.text})"))
                    .params(search_term=normalized)
                )
            )
    ...
    def _apply_filters(self, query: Select, params: TransactionListQuery) -> Select:
        ...
        if params.tags:
            ...
            for tag in params.tags:
                tag_conditions.append(Transaction.tags.like(f"%{tag}%"))  # type: ignore[arg-type]
    ...
    def _apply_sorting(self, query: Select, sort_param: Optional[str], has_search: bool) -> Select:
        ...
        if direction == "desc":
            query = query.order_by(column.desc(), Transaction.id.desc())  # type: ignore[attr-defined]
        else:
            query = query.order_by(column.asc(), Transaction.id.asc())  # type: ignore[attr-defined]

        return query

    def _apply_pagination(self, query: Select, page: int, limit: int) -> Select:
        """Apply pagination with limit and offset."""
        offset = (page - 1) * limit
        query = query.limit(limit).offset(offset)
        return query

    async def build_list_query(self, params: TransactionListQuery) -> Tuple[List[Transaction], int]:
